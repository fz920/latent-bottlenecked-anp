...
def main():
    ...
    parser.add_argument('--num_tar', type=int, default=None)
    parser.add_argument('--custom_eval', default=False)
    ...
    if args.mode == 'train':
        ...
        profile_memory(args, model, args.num_ctx)
        
        

# Define your function for memory profiling
@profile
def profile_memory(args, model, num_ctx):
    path, filename = get_eval_path(args)
    if not osp.isfile(osp.join(path, filename)):
        print('generating evaluation sets...')
        gen_evalset(args)

    print(f"Starting evaluation with {num_ctx} context points")
    ...
    print("Evaluation with", num_ctx, "context points has finished")
...
# Function to run nvidia-smi and get GPU memory usage
import subprocess
...
def eval(args, model):
    ...
    with torch.no_grad():
        for batch in tqdm(eval_batches, ascii=True):
            ...
            #empty_cache()
            #initial_memory = torch.cuda.memory_allocated('cuda')
            ...
            #final_memory = torch.cuda.memory_allocated('cuda')
            #memory_used = final_memory - initial_memory
            #memory_used_MB = memory_used / (1024 * 1024)
            #print(f"Memory used for inference: {memory_used_MB} MB")
            ...
            #    print("Execution time:", curr_time/len(eval_batches), "miliseconds per batch")
            #    #track time and memory
            ...
            #    print("Memory Usage Change:", (final_memory - initial_memory) / (1024 ** 2) /len(eval_batches), "MB")
